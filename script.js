...
function updateTotal(event){
    ...
    total_field.children[0].innerText = number_of_items.value * price_field_content + 'DT' 
...
function grandTotal(){
    ...
    for ( let i = 0; i < total_price.length; i++) {
       total_price_content = Number(total_price[i].innerText.replace ( 'DT' , '') )
       total += total_price_content
    }
    grand_total.children[0].innerText= total + 'DT'
